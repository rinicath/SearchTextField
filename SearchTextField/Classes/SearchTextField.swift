    ...
    fileprivate func redrawSearchTableView() {
        ...
        if let tableView = tableView {
            ...
            if self.direction == .down || self.direction == .auto {
                ...
                let tableHeight = min((tableView.contentSize.height), (frame.origin.y - theme.padding))
                UIView.animate(withDuration: 0.2, animations: { [weak self] in
                    self?.tableView?.frame = CGRect(x: frame.origin.x + 2, y: (frame.origin.y - tableHeight - (self?.theme.padding)!), width: frame.size.width - 4, height: tableHeight)
...
extension SearchTextField: UITableViewDelegate, UITableViewDataSource {
    ...
    open func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if cell == nil {
            cell = UITableViewCell(style: .value1, reuseIdentifier: SearchTextField.cellIdentifier)
        }
        ...
        cell!.detailTextLabel?.font = UIFont(name:"Avenir", size:22)//UIFont(name: theme.font.fontName, size: theme.font.pointSize * fontConversionRate)
